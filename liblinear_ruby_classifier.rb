...
$map_index_to_catagory = $preprocessor.categories.invert

def SVMclassifier text
  xs = text.split.collect {|x| $preprocessor.hash_of_ngrams[[x]] || -1}
  xs = xs.select {|x| x > -1}
  h = {}
  xs.each {|x| h[x] = (h[x] || 0) + 1}
  $map_index_to_catagory[$model.predict(h)]
end
